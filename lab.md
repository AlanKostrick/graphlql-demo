...
2. Add a new `mutation` in the `type Mutation` for `addCourse` ... think about the information you will need for a couse. 
    ...
    - The `id` should be autogenerated... lets head to the `server` terminal and `npm install uuid` so that our server can autogenerate these for us, taking the responsibility off of the user 
